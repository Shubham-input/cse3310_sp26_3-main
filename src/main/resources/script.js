...
function logoutButton() {
  ...
  updateOfflineUsers(offlineUsers);
}
  ...
  users.forEach((u) => {
    if (blockedUsers.includes(u)) {
      ...
      <div class="online-user">
        <span class="username" style="color:red; text-decoration:line-through;">${u}</span>
        <button class="block-btn" onclick="toggleBlock('${u}')">Unblock</button>
      </div>`;
    } else {
      div.innerHTML += `
      <div class="online-user">
        <span class="username">${u}</span>
        <button class="block-btn" onclick="toggleBlock('${u}')">Block</button>
      </div>`;
    }
  });
}
...
function react(postId, type) {
  ...
  const previousReaction = post.reactedBy[currentUser];
  if (previousReaction === type) {
    post[type] = Math.max(0, post[type] - 1);
    delete post.reactedBy[currentUser];
  } else if (previousReaction) {
    post[previousReaction] = Math.max(0, post[previousReaction] - 1);
    post[type] = (post[type] || 0) + 1;
    post.reactedBy[currentUser] = type;
  } else {
    post[type] = (post[type] || 0) + 1;
    post.reactedBy[currentUser] = type;
  }
